...
export default function CameraScreen2(){
  ...
  const pan1 = useRef(new Animated.ValueXY()).current;
    ...
    const newValue = gestureState.moveY
    setValueY1(newValue)
  ...
  const pan2 = useRef(new Animated.ValueXY()).current;
    ...
    onPanResponderRelease: (event, gestureState) => {
        ...
        const newValue2 = gestureState.moveY
    setValueY2(newValue2)
      }
    })
  ).current;
...
<Text style={styles.textStyle1}>{valueY1}</Text>

<Text style={styles.textStyle2}>{valueY2}</Text>
